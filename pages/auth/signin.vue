...
const url =
  ...
  "MyInterviewAdvisor/public/api/login";
const onSubmit = () => {
  ...
  data.append("email", formData.value[0].value),
    ...
    axios(url, {
      ...
      .then(async (res) => {
        ...
        useRouter().push(`/exercise/${Number(res.data.user.step) || "1"}`);
      })
      .catch((error) => {
  ...
  <Layout>
    <template #header>
      ...
      <p class="text-white/90 font-medium leading-relaxed text-center text-xs">
        We've seen it time and again. The candidates who excel in job interviews
        are the ones who prepare. Preparation doesn't mean reading an article or
        two. It means being intentional about crafting your answers, designing
        your examples, and above all, practicing.
      </p>
      <p class="text-white/90 font-medium leading-relaxed text-center text-xs">
            ...
            <div v-for="field in formData" :key="field.name" class="mt-6">
              <div class="flex justify-between mb-2">
                ...
                <a
                  ...
                  class="text-sm text-gray-500 focus:text-blue-500 hover:text-blue-500 hover:underline"
                  >Forgot password?</a
                >
              </div>
              <input
